...
static void PhysicsManager_DetermineCollisionPointConvexHullFace(Vector* dest,
	const DynamicArray* furthestOnHull1, const DynamicArray* furthestOnHull2);
...
static unsigned char PhysicsManager_IsResolutionNeeded(struct Collision* collision)
{
	...
	if(collision->obj1->body && collision->obj2->body)
	{
		...
		if(collision->obj1->collider->type == COLLIDER_CONVEXHULL)
		{
			...
			struct ColliderData_ConvexHull* convex = collision->obj1->collider->data->convexHullData;
		...
		if(collision->obj2->collider->type == COLLIDER_CONVEXHULL)
		{
			...
			//Grab convexHull data set
			struct ColliderData_ConvexHull* convex = collision->obj2->collider->data->convexHullData;
...
static void PhysicsManager_ApplyFrictionalTorques(struct Collision* collision, const float staticCoefficient, const float dynamicCoefficient)
{
	...
	if(collision->obj1->body != NULL)
	{
		...
		else
		{
			...
			Vector_GetScalarProduct(&frictionalTorque, &axis, -1.0f * dynamicMag1);
