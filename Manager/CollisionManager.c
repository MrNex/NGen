...
void CollisionManager_TestAABBSphereCollision(struct Collision* dest, GObject* AABBObj, FrameOfReference* AABBFoR, GObject* sphereObj, FrameOfReference* sphereFoR)
{
	...
	//Test overlap of opposite bounds on each axis
	//If the overlap is negative on any axis, there is no collision.
	//If the overlap is zero on any axis there is contact
	...
	//The MTV must always point towards obj1. The MTV will be the face normal of the bounding box,
	//Therefore obj1 must be the object with the sphere collider
	dest->obj1 = sphereObj;
