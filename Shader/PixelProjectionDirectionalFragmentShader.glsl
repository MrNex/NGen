...
//Returns:
//
vec4 CalculateDirectionalLight(vec3 worldPos, vec3 worldNormal, float shadow)
{
	...
	vec4 diffuseColor = (1.0f - shadow) * vec4(diffuseIntensity * diffuseFactor * lightColor, 1.0f);
...
void main()
{
	...
	float shadow = texture(shadowTexture, textureCoordinate).x;

	gl_FragColor = vec4(diffuseColor, 1.0f) * CalculateDirectionalLight(worldPos, worldNormal, shadow);
