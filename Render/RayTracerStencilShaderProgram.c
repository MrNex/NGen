...
struct RayTracerStencilShaderProgram_Members
{
...
///
//Allocates a set of variables needed by a RayTracerStencilShaderProgram
//
//Returns:
//	a (void) pointer to a struct containing the set of uniforms/members the shader program needs.
static ShaderProgram_Members RayTracerStencilShaderProgram_AllocateMembers(void);
...
static void RayTracerStencilShaderProgram_InitializeMembers(ShaderProgram* prog);

///
//Frees the memory used up by a RayTracerStencilShaderProgram's members
//
//Parameters:
//	prog: A pointer to the shader program to free the members of.
static void RayTracerStencilShaderProgram_FreeMembers(ShaderProgram* prog);
...
//
//Parameters:
//	prog: A pointer to the raytracer setncil shader program to set the uniform variables of
...
static void RayTracerStencilShaderProgram_SetVariableUniforms(ShaderProgram* prog, Camera* cam, GObject* gameObj);

///
//Renders stencils of point lights to the depth buffers stencil component
//
//Parameters:
//	prog: A pointer to the RayTracerStencilShaderProgram with which to render
...
static void RayTracerStencilShaderProgram_Render(ShaderProgram* prog, RenderingBuffer* buffer, GObject* gObject);


///
//Initializes the raytracer stencil shader program
//
//Parameters:
//	prog: A pointer to the shader program
void RayTracerStencilShaderProgram_Initialize(ShaderProgram* prog)
{
	ShaderProgram_Initialize(prog, "Shader/RayTracerPointVertexShader.glsl", "Shader/NullFragmentShader.glsl");

	prog->members = RayTracerStencilShaderProgram_AllocateMembers();
	RayTracerStencilShaderProgram_InitializeMembers(prog);

	prog->FreeMembers = RayTracerStencilShaderProgram_FreeMembers;
	prog->Render = (ShaderProgram_RenderFunc)RayTracerStencilShaderProgram_Render;
}
...
static ShaderProgram_Members RayTracerStencilShaderProgram_AllocateMembers(void)
{
	return malloc(sizeof(struct RayTracerStencilShaderProgram_Members));
}
...
static void RayTracerStencilShaderProgram_InitializeMembers(ShaderProgram* prog)
{
	struct RayTracerStencilShaderProgram_Members* members = prog->members;
...
static void RayTracerStencilShaderProgram_Render(ShaderProgram* prog, RenderingBuffer* buffer, GObject* gObject)
{
	...
	RayTracerStencilShaderProgram_SetVariableUniforms(prog, buffer->camera, gObject);
