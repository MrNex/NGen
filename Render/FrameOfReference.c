...
{
	FrameOfReference_Initialize(copy);
	Matrix_Copy(copy->scale, original->scale);
	Matrix_Copy(copy->rotation, original->rotation);
	Vector_Copy(copy->position, original->position);
}
